#!/bin/bash
set -e  # Arrêter en cas d'erreur

echo "========================================="
echo "Initialisation de l'utilisateur HAPI PostgreSQL"
echo "========================================="

# Variables avec valeurs par défaut si non définies
HAPI_USER="${HAPI_USER:-hapi_user}"
HAPI_PASSWORD="${HAPI_PASSWORD:-hapi_user_123}"
#DB_NAME="${HAPI_DB:-hapi}"
#DB_GRANT_SUPERUSER="${APP_GRANT_SUPERUSER:-false}"
#DB_CONNECTION_LIMIT="${APP_CONNECTION_LIMIT:--1}"  # -1 = illimité

echo "Configuration HAPI:"
echo "  - Utilisateur: $HAPI_USER"
echo "  - Base de données: hapi"
#echo "  - Superuser: $DB_GRANT_SUPERUSER"
#echo "  - Limite de connexions: $DB_CONNECTION_LIMIT"
echo ""

# Fonction pour vérifier si un utilisateur existe
user_exists() {
    psql -U "$POSTGRES_USER" -tAc "SELECT 1 FROM pg_user WHERE usename='$1'" | grep -q 1
}

# Fonction pour vérifier si une base existe
database_exists() {
    psql -U "$POSTGRES_USER" -tAc "SELECT 1 FROM pg_database WHERE datname='$1'" | grep -q 1
}

# Créer l'utilisateur
if user_exists "$HAPI_USER"; then
    echo "⚠️  L'utilisateur $HAPI_USER existe déjà, mise à jour du mot de passe..."
    psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname postgres <<-EOSQL
        ALTER USER ${HAPI_USER} WITH PASSWORD '${HAPI_PASSWORD}';
EOSQL
else
    echo "✅ Création de l'utilisateur $HAPI_USER..."
    
    # Construire la commande CREATE USER selon les options
    if [ "$HAPI_GRANT_SUPERUSER" = "true" ]; then
        psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname postgres <<-EOSQL
            CREATE USER ${HAPI_USER} WITH 
                PASSWORD '${HAPI_PASSWORD}'
                SUPERUSER
                CREATEDB
                CREATEROLE;
EOSQL
    else
        psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname postgres <<-EOSQL
            CREATE USER ${HAPI_USER} WITH 
                PASSWORD '${HAPI_PASSWORD}'
                NOSUPERUSER
                NOCREATEDB
                NOCREATEROLE;
EOSQL
    fi
fi

# Créer la base de données
if database_exists hapi; then
    echo "⚠️  La base de données hapi existe déjà"
else
    echo "✅ Création de la base de données hapi..."
    psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname postgres <<-EOSQL
        CREATE DATABASE hapi OWNER ${HAPI_USER};
EOSQL
fi

# Accorder les privilèges
echo "✅ Configuration des privilèges..."
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname postgres <<-EOSQL
    -- Privilèges sur la base de données
    GRANT ALL PRIVILEGES ON DATABASE hapi TO ${HAPI_USER};
EOSQL

# Se connecter à la nouvelle base pour configurer les permissions du schéma
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname postgres <<-EOSQL
    -- Privilèges sur le schéma public
    GRANT ALL ON SCHEMA public TO ${HAPI_USER};
    
    -- Privilèges par défaut pour les futures tables
    ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL ON TABLES TO ${HAPI_USER};
    ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL ON SEQUENCES TO ${HAPI_USER};
    ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL ON FUNCTIONS TO ${HAPI_USER};
    
    -- Si des tables existent déjà, donner les permissions
    GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO ${HAPI_USER};
    GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public TO ${HAPI_USER};
    GRANT ALL PRIVILEGES ON ALL FUNCTIONS IN SCHEMA public TO ${HAPI_USER};
EOSQL

echo ""
echo "========================================="
echo "✅ Initialisation HAPI terminée avec succès!"
echo "========================================="
echo ""
echo "Connexion possible avec:"
echo "  psql -h localhost -U $HAPI_USER -d hapi"
echo ""