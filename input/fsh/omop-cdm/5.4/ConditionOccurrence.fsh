Logical: OMOPConditionOccurrence
Parent: Base
...
* condition_type_concept_id 1..1 Reference(OMOPConcept) "Condition Type" "This field can be used to determine the provenance of the Condition record, as in whether the condition was from an EHR system, insurance claim, registry, or other sources."
* condition_status_concept_id 0..1 Reference(OMOPConcept) "Condition Status" "This concept represents the point during the visit the diagnosis was given (admitting diagnosis, final diagnosis), whether the diagnosis was determined due to laboratory findings, if the diagnosis was exclusionary, or if it was a preliminary diagnosis, among others."
...
* provider_id 0..1 Reference(OMOPProvider) "Provider" "The provider associated with condition record, e.g. the provider who made the diagnosis or the provider who recorded the symptom."
* visit_occurrence_id 0..1 Reference(OMOPVisitOccurrence) "Visit Occurrence" "The visit during which the condition occurred."
* visit_detail_id 0..1 Reference(OMOPVisitDetail) "Visit Detail" "The VISIT_DETAIL record during which the condition occurred. For example, if the person was in the ICU at the time of the diagnosis the VISIT_OCCURRENCE record would reflect the overall hospital stay and the VISIT_DETAIL record would reflect the ICU stay during the hospital visit."
...
* condition_source_concept_id 0..1 Reference(OMOPConcept) "Condition Source Concept" "This is the concept representing the condition source value and may not necessarily be standard. This field is discouraged from use in analysis because it is not required to contain Standard Concepts that are used across the OHDSI community, and should only be used when Standard Concepts do not adequately represent the source detail for the Condition necessary for a given analytic use case. Consider using CONDITION_CONCEPT_ID instead to enable standardized analytics that can be consistent across the network."
