...
* observation_id 1..1 string "Observation Identifier" 
* person_id 1..1 Reference(OMOPPerson) "Person" "The PERSON_ID of the Person for whom the Observation is recorded. This may be a system generated code."
* observation_concept_id 1..1 Reference(OMOPConcept) "Observation" "The OBSERVATION_CONCEPT_ID field is recommended for primary use in analyses, and must be used for network studies."
...
* observation_type_concept_id 1..1 Reference(OMOPConcept) "Observation Type" "This field can be used to determine the provenance of the Observation record, as in whether the measurement was from an EHR system, insurance claim, registry, or other sources."
...
* value_as_concept_id 0..1 Reference(OMOPConcept) "Value as Concept" "It is possible that some records destined for the Observation table have two clinical ideas represented in one source code. This is common with ICD10 codes that describe a family history of some Condition, for example. In OMOP the Vocabulary breaks these two clinical ideas into two codes; one becomes the OBSERVATION_CONCEPT_ID and the other becomes the VALUE_AS_CONCEPT_ID. It is important when using the Observation table to keep this possibility in mind and to examine the VALUE_AS_CONCEPT_ID field for relevant information."
* qualifier_concept_id 0..1 Reference(OMOPConcept) "Qualifier" "This field contains all attributes specifying the clinical fact further, such as as degrees, severities, drug-drug interaction alerts etc."
* unit_concept_id 0..1 Reference(OMOPConcept) "Unit" "There is currently no recommended unit for individual observation concepts. UNIT_SOURCE_VALUES should be mapped to a Standard Concept in the Unit domain that best represents the unit as given in the source data."
* provider_id 0..1 Reference(OMOPProvider) "Provider" "The provider associated with the observation record, e.g. the provider who ordered the test or the provider who recorded the result."
* visit_occurrence_id 0..1 Reference(OMOPVisitOccurrence) "Visit Occurence" "The visit during which the Observation occurred."
* visit_detail_id 0..1 Reference(OMOPVisitDetail) "Visit Detail" "The VISIT_DETAIL record during which the Observation occurred. For example, if the Person was in the ICU at the time the VISIT_OCCURRENCE record would reflect the overall hospital stay and the VISIT_DETAIL record would reflect the ICU stay during the hospital visit."
...
* observation_source_concept_id 0..1 Reference(OMOPConcept) "Observation Source" "This is the concept representing the OBSERVATION_SOURCE_VALUE and may not necessarily be standard. This field is discouraged from use in analysis because it is not required to contain Standard Concepts that are used across the OHDSI community, and should only be used when Standard Concepts do not adequately represent the source detail for the Observation necessary for a given analytic use case. Consider using OBSERVATION_CONCEPT_ID instead to enable standardized analytics that can be consistent across the network."
...
//* observation_event_id 0..1 Reference(OMOPObservationEvent) "Observation Event" 
...
* obs_event_field_concept_id 0..1 Reference(OMOPConcept) "Observation Event Field Concept" "If the Observation record is related to another record in the database, this field is the CONCEPT_ID that identifies which table the primary key of the linked record came from."
