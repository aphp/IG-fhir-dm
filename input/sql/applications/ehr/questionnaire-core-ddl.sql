...
DROP TABLE IF EXISTS patient_addresse CASCADE;
...
CREATE TABLE patient_addresse (
    patient_addresse_id BIGSERIAL PRIMARY KEY,
...
ADD CONSTRAINT fk_patient_location_patient 
...
CHECK (date_heure_fin IS NULL OR date_heure_debut IS NULL OR date_fin_prescription >= date_heure_fin);
...
CREATE INDEX idx_patient_loc_iris ON patient_location(code_iris) WHERE code_iris IS NOT NULL;
...
CREATE INDEX idx_exposition_patient_atc ON prescription(patient_id, code_atc);
...
CREATE INDEX idx_patient_coords ON patient_location(latitude, longitude) WHERE latitude IS NOT NULL AND longitude IS NOT NULL;
...
COMMENT ON COLUMN patient_location.latitude IS 'Latitude du domicile patient (linkId: 3709843054556)';
COMMENT ON COLUMN patient_location.longitude IS 'Longitude du domicile patient (linkId: 7651448032665)';
