...
- **`input/test`**: this directory contains the files used to test the workflow, in particular the loading of the EHR database, and to verify that the StructureMap produces the expected results.

Two more top-level directories, outside `input`, matter for this repo:

- **`data-platform/`**: the Health Data Platform implementation — `docker/`, `raw-layer/` (PostgreSQL EHR schema + Docker Compose + Python test-data loader), `semantic-layer/`, `share-layer/`, and `transform-layer/` (a DBT project, `transform-layer/data-transformer/`, for building and managing the data transformations). SQL scripts for the data models live under each of these subfolders' own `sql/`/`docker/init` directories, not in a single top-level location.
- **`order/`**: reusable task-prompt templates for recurring authoring workflows in this repo (e.g. `GenerateProvenanceOfFHIRResource.md`, `GenerateFMLQuestionnaire2FHIR.md`).

## Important Commands

- `./gradlew buildIG` - Full build: SUSHI + IG Publisher, produces `output/package.tgz`
- `./gradlew sushiBuild` - Fast FSH-only check, useful to test new fsh files
- `./gradlew reBuildIG` - Full clean + full build (slow; only when you have reason to distrust the current build state)
- `./gradlew cleanIG` - Deletes `fsh-generated/`, `output/`, `input-cache/`, cached publisher/manager jars, `node_modules/` — a real, sometimes-slow reset, not a free wipe
- ⚠️ **Never run bare `gradle`/`./gradlew` with no task name** — `build.gradle.kts` sets `defaultTasks("cleanIG", "buildIG")`, so a bare invocation silently triggers the slowest possible full clean-and-rebuild. Always name the task explicitly.
- SQL scripts for testing/building the data models are scattered under `data-platform/*/sql` (and `data-platform/*/docker/init`), plus `input/test/sql` for test fixtures — there is no single canonical SQL entry point; `psql -U postgres -f <path-to-the-relevant-script>.sql` against whichever applies.
